...
\text{such that}\;\;&y_i(\bw^T\x_i + b) - 1 \geq 0\\
\text{and} \;\;&p_i \geq 0
\end{align*}

\qu What does the $i$ iterate over? How many terms does the sum have, and how many constraints are there?

\ans{It iterates over the data. There are as many terms and constraints as there are instances in the data.}{}
...
\footnotetext{For plain Lagrange multipliers, where the constraints are all equalities, we can add or subtract the term containing the constraint.}
...
\ans{
The solutions follow directly from (\ref{line:constraint}), above. If $a =0$, then $b^2=1$ and vice versa. This gives us
\begin{align*}
a = 0, b = 1	
a = 0, b = -1
a = 1, b = 0	
...
\begin{align*}
k(\ba, \bb) &= \left( \ma{a_1\\ a_2}^T\ma{b_1\\ b2} \right)^2\\
...
k(\ba, \bb) &= \left( \ma{a_1\\ a_2}^T\ma{b_1\\ b2} + 1\right)^2\\
 ...
 &= \ma{1 \\ \sqrt{2}a_1 \\ {a_1}^2\\\sqrt{2}a_1a_2 \\{a_2}^2} ^T\ma{1 \\ \sqrt{2}b_1 \\ {b_1}^2\\\sqrt{2}b_1b_2 \\{b_2}^2} 
\end{align*}
}{}
...
p(0) &= \frac{1}{2} N(0, 1) + \frac{1}{2} N(1,1)\\
 ...
 &= \frac{1}{\sqrt{8\pi e}} \approx 0.12
\end{align*}
}{}
...
\[
\frac{\frac{1}{2\sqrt{2\pi}} \exp(0)}{\frac{1}{2\sqrt{2\pi}} \exp(0) + \frac{1}{2\sqrt{2\pi}} \exp\left(-\frac{1}{2}\right)} = \frac{1}{1 + \frac{1}{\sqrt{e}}}
\]
and for component 2: 
\[
\frac{\frac{1}{2\sqrt{2\pi}} \exp\left(-\frac{1}{2}\right)}{\frac{1}{2\sqrt{2\pi}} \exp(0) + \frac{1}{2\sqrt{2\pi}} \exp\left(-\frac{1}{2}\right)} = \frac{\frac{1}{\sqrt{e}}}{1 + \frac{1}{\sqrt{e}}}
\]
}{}
